...
def extract_abgabetermin(txt):
    ...
    if not m2: return {"raw": line.strip()}
...
def extract_besichtigung(txt):
    ...
    treff = None
    if tail:
        # einfacher Heuristik-Treffpunkt
        mm = re.search(r"Treffpunkt\s+ist\s+(?P<p>.+?)(?:\.|\n)", tail)
        treff = mm.group('p').strip() if mm else None
...
def extract_contacts(txt):
    ...
    for em in re.finditer(EMAIL, txt):
        span = (max(0, em.start()-120), min(len(txt), em.end()+120))
        window = txt[span[0]:span[1]]
        tel = re.search(PHONE, window)
        name = None
        # Name-Heuristik: Zeile vor E-Mail
        lines = window.splitlines()
        for i,ln in enumerate(lines):
            if em.group(0) in ln and i>0:
                prev = lines[i-1].strip()
                if 2 <= len(prev.split()) <= 6:
                    name = prev
        contacts.append({
            "name": name,
...
def extract_kriterien(txt):
    block = re.search(r"(?is)4\s+Beurteilungskriterien.*?(?P<li>(?:\n\s*•.+)+)", txt)
    if not block: return []
    return [re.sub(r"^\s*•\s*", "", ln).strip()
            for ln in block.group('li').splitlines() if ln.strip().startswith("•")]
...
def extract_teilnehmende(txt):
    ...
    # filtern auf Zeilen mit Buchstaben
